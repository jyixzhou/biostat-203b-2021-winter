...
icustays_tble %>% 
  ...
  show_query() %>% 
  collect() %>% 
  saveRDS(file = "~/biostat-203b-2021-winter/hw3/mimiciv_shiny/icu_cohort.rds")
  # mutate(total_los <- difftime(deathtime, admittime, units = "days")) %>% 
  # mutate(death_within_30days = ifelse(hospital_expire_flag == 1,
  #   as.numeric(deathtime - admittime) <= 30, FALSE)) %>% 
  # print(width = Inf)

saveRDS(subset, file = "~/biostat-203b-2021-winter/hw3/mimiciv_shiny/icu_cohort.rds")
```
