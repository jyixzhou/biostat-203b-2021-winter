...
```{r read in d_labitems, eval=FALSE}
...
```{r read in labevents, eval=FALSE}
if  (!file.exists("labevents_icustays.csv.gz")) {
  labevents_data <-
    fread(str_c(mimic_path, "/hosp/labevents.csv.gz"),
          select = c("subject_id", "hadm_id", "itemid", "charttime", "valuenum"), nThread = 4)
  labevents_data %>% 
    semi_join(icustays_data, by = c("subject_id", "hadm_id")) %>% 
    fwrite("labevents_icustays.csv.gz", nThread = 4)
}
lab_events_data <- read_csv("labevents_icustays.csv.gz")
head(lab_events_data)
```

```{r filter for lab items of interest, eval=FALSE}
labs_of_interest <- 
  c(50912, 50971, 50983, 50902, 50882, 51221, 51301, 50931, 50960, 50893, 50813)
lab_events_data <- lab_events_data %>% 
  filter(itemid %in% labs_of_interest)

lab_events_data %>% head()

#lab_events_data %>% fwrite("labevents_icustays_selected.csv.gz")
```
...
First read in the `d_items.csv.gz` and `chartevents.csv.gz` data.
...
```{r read in d_items, eval=FALSE}
d_items_data <- read_csv(str_c(mimic_path, '/icu/d_items.csv.gz'))
head(d_items_data)
d_items_data %>% 
  filter(str_detect(label, regex("blood pressure", ignore_case = T))) %>% 
  filter(str_detect(label, regex("systolic", ignore_case = T))) %>% 
  print(width = Inf)
d_items_data %>% 
  filter(str_detect(label, regex("SpO2", ignore_case = T))) %>% 
  print(width = Inf)
d_items_data %>% 
  filter(str_detect(label, regex("respiratory", ignore_case = T))) %>% 
  print(width = Inf)
```
...
```{r read in chartevents, eval=FALSE}
